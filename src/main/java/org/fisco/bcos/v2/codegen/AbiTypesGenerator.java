...
public class AbiTypesGenerator extends Generator {
    ...
    private <T extends Type> void generateBytesTypes(Class<T> superclass, String path)
            throws IOException {
        String packageName = createPackageName(superclass);
        ClassName className;
    ...
    private <T extends Type> void generateStaticArrayTypes(Class<T> superclass, String path)
            throws IOException {
        String packageName = createPackageName(superclass);
        ClassName className;
        ...
        for (int length = 1; length <= StaticArray.MAX_SIZE_OF_STATIC_ARRAY; length++) {
            ...
            TypeSpec bytesType =
                    TypeSpec.classBuilder(className.simpleName())
                            ...
                            .superclass(
                                    ParameterizedTypeName.get(
                                            ClassName.get(superclass), typeVariableName))
                            .addModifiers(Modifier.PUBLIC)
                            .addMethods(
