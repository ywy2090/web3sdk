package org.fisco.bcos.v2.crypto.sm.sm2.crypto.digests;

import org.fisco.bcos.v2.crypto.sm.sm2.util.Memoable;
import org.fisco.bcos.v2.crypto.sm.sm2.util.Pack;

/**
 * Implementation of Chinese SM3 digest as described at
 * http://tools.ietf.org/html/draft-shen-sm3-hash-00 and at .... ( Chinese PDF )
 *
 * <p>The specification says "process a bit stream", but this is written to process bytes in blocks
 * of 4, meaning this will process 32-bit word groups. But so do also most other digest
 * specifications, including the SHA-256 which was a origin for this specification.
 */
public class SM3Digest extends GeneralDigest {
     ...
     *
     *
     * Tj = 79cc4519 when 0 < = j < = 15 Tj = 7a879d8a when 16 < = j < = 63
     *
     * 3.4.3. Boolean function
     *
     *
     * FFj(X;Y;Z) = X XOR Y XOR Z when 0 < = j < = 15 = (X AND Y) OR (X AND Z)
     * OR (Y AND Z) when 16 < = j < = 63
     *
     * GGj(X;Y;Z) = X XOR Y XOR Z when 0 < = j < = 15 = (X AND Y) OR (NOT X AND
     * Z) when 16 < = j < = 63
     *
     ...
     *
     *
     * P0(X) = X XOR (X <<< 9) XOR (X <<< 17) ## ROLL, not SHIFT P1(X) = X XOR
     * (X <<< 15) XOR (X <<< 23) ## ROLL, not SHIFT
     *
     ...
     *
     * ROLL 9 : ((x << 9) | (x >>> (32-9)))) ROLL 17 : ((x << 17) | (x >>>
     * (32-17))) ROLL 15 : ((x << 15) | (x >>> (32-15))) ROLL 23 : ((x << 23) |
     * (x >>> (32-23)))
     */
