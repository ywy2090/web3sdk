...
import org.fisco.bcos.v2.precompile.exception.PrecompileMessageException;
...
import org.fisco.bcos.v2.web3j.tx.gas.DefaultGasProvider;
...
public class CnsService {
    ...
    public String getAddressByContractNameAndVersion(String contractNameAndVersion) {
        ...
        if (!WalletUtils.isValidAddress(address)) {
            throw new RuntimeException(
                    "Unable to resolve address for name: " + contractNameAndVersion);
        } else {
            return address;
        }
    }
    ...
    boolean isSynced() throws Exception {
        ...
        if (ethSyncing.isSyncing()) {
            ...
            BcosBlock block =
                    web3j.getBlockByNumber(new BigInteger("1"), false).send();
